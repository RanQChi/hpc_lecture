...
//----------------------------  1  -----------------------------
  ...
  for(int i=0; i<2; i++) offset[i] *= Nlocal[i];
	...
	hsize_t new_Nlocal[2] = {1, 1};
  	hsize_t new_offset[2] = {mpirank / dim[0], mpirank % dim[0]};
//----------------------------  1  -----------------------------


//----------------------------  2  ---------------------------
  hsize_t count[2] = {1,1};
  hsize_t stride[2] = {mpirank / dim[0], mpirank % dim[0]};

	hsize_t new_count[2] = {NX/dim[0], NX/dim[1]};
	hsize_t new_stride[2] = {2, 2};
//----------------------------  2  -----------------------------
...
//--------------------------------  3  ------------------------------------------------
  //H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal);
  	...
  	H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, new_offset, new_stride, new_count, new_Nlocal);
//--------------------------------  3  -----------------------------------------------
  H5Pclose(plist);
